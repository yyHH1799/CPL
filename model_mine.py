...
class visible_module(nn.Module):
    ...
    def forward(self, x):
        if self.share_net == 0:
            ...
            if self.share_net > 1:
                for i in range(1, self.share_net):
                    x1 = getattr(self.visible, 'layer' + str(i))(x)
...
class thermal_module(nn.Module):
    ...
    def forward(self, x):
        if self.share_net == 0:
            ...
            if self.share_net > 1:
                for i in range(1, self.share_net):
                    x1 = getattr(self.thermal, 'layer' + str(i))(x)
...
class base_resnet(nn.Module):
    ...
    def forward(self, x):
        if self.share_net == 0:
            ...
            x1 = getattr(self.base, 'layer' + str(2))(x)

            x2 = getattr(self.base, 'layer' + str(3))(x1)

            x3 = getattr(self.base, 'layer' + str(4))(x2)
    ...
    def forward(self, x1, x2, modal=0):
        ...
        if self.pcb == 'on':
            ...
            for i in range(self.num_stripes):
                ...
                local_feat = self.local_conv_list[i](
                    local_feat.view(feat.size(0), feat.size(1), 1, 1))
            ...
            if self.training:
                ...
                return self.l2norm(feat_all), local_feat_list, logits_list, feat_all
        else:
            if self.gm_pool == 'on':
                b, c, h, w = x.shape
                x = x.view(b, c, -1)
