...
def extract_gall_feat(gall_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(gall_loader):
            ...
            if args.pcb == 'on':
                feat_pool, _, _, _= net(input, input, test_mode[0])
...
def extract_query_feat(query_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(query_loader):
            ...
            input = Variable(input.cuda())
            if args.pcb == 'on':
                feat_pool, _, _, _ = net(input, input, test_mode[1])
...
if dataset == 'sysu':
    ...
    for trial in range(10):
        ...
        model_path = checkpoint_path + str('10') + '/' +'regdb_c_tri_pcb_on_w_tri_2.0_s6_f256_share_net2_base_gm10_k4_p8_lr_0.01_seed_0_trial_10_best.t'.format(
            test_trial)
        if os.path.isfile(model_path):
            print('==> loading checkpoint {}'.format(args.resume))
            checkpoint = torch.load(model_path)
